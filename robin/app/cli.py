...
def _load_json_contracts(path: Path, contract_type: type[BacktestRun] | type[StrategyDecision]) -> list[Any]:
    if not path.exists():
        return []
    return [contract_type.model_validate(json.loads(item.read_text(encoding="utf-8"))) for item in sorted(path.glob("*.json"))]


@ingest_app.command("run")
def ingest_run(
    registry: Path = typer.Option(_default_fixture("source_registry.yaml"), "--registry", help="Source registry YAML."),
    ...
    lake.initialize()
    if dry_run:
        _echo({"dry_run": True, "registry": str(registry), "lake_root": str(config.lake_root)})
        return
...
def facts_build_evidence(
    security_master: Path = typer.Option(_default_fixture("security_master.csv"), "--security-master"),
    ...
    packets = build_evidence_packets(raw_documents, SecurityMaster.from_csv(security_master))
    if not dry_run:
        lake.write_evidence_packets(packets)
    _echo({"dry_run": dry_run, "raw_documents": len(raw_documents), "evidence_packets": len(packets), "packet_ids": [packet.id for packet in packets]})
...
def features_compute_daily(
    ...
    run_date = date.fromisoformat(as_of) if as_of else frame.select(pl.col("date").max()).item()
    ...
    _echo({"dry_run": dry_run, "as_of": run_date.isoformat(), "factor_values": len(values), "path": path})
...
def features_backfill(
    ...
    if dry_run:
        _echo({"dry_run": True, "start": start_date.isoformat(), "end": end_date.isoformat(), "symbols": frame.get_column("symbol").n_unique()})
        return
...
def replay_run(
    registry: Path = typer.Option(_default_fixture("source_registry.yaml"), "--registry"),
    security_master: Path = typer.Option(_default_fixture("security_master.csv"), "--security-master"),
    ohlcv: Path = typer.Option(_default_fixture("ohlcv.csv"), "--ohlcv"),
    root: Path = typer.Option(Path("."), "--root"),
    dry_run: bool = typer.Option(False, "--dry-run"),
    ...
    lake = DataLake(config)
    lake.initialize()
    ingest_result = {"dry_run": True} if dry_run else run_ingest_from_registry_path(config, registry)
    ...
    if not dry_run:
        lake.write_evidence_packets(packets)
        write_factor_values(factor_values, config.lake_root / config.lake.gold_path / "factor_values")
    replay_hash = stable_hash(
        {"raw": [doc.id for doc in raw_documents], "packets": [packet.id for packet in packets], "factors": [value.id for value in factor_values]}
    )
    _echo(
        {
            "dry_run": dry_run,
...
def backtest_run(
    ...
    run = run_factor_backtest(frame, "vnext_fixture_strategy", factor, artifact_dir=config.lake_root / config.lake.artifact_path / "backtests")
    path = ""
    mlflow_uri = ""
    if not dry_run:
        path = str(write_backtest_report(run, config.lake_root / config.lake.artifact_path / "backtests"))
        mlflow_uri = _log_backtest_mlflow(run, config)
    _echo({"dry_run": dry_run, "backtest_run_id": run.id, "gross": run.gross_metrics, "net": run.net_metrics, "path": path, "mlflow_uri": mlflow_uri})
...
def _log_backtest_mlflow(run: BacktestRun, config: VNextConfig) -> str:
    try:
        ...
        tracking_uri = (config.lake_root / config.lake.artifact_path / "mlruns").resolve().as_uri()
        mlflow.set_tracking_uri(tracking_uri)
        mlflow.set_experiment("robin-vnext-local")
        with mlflow.start_run(run_name=run.id):
            mlflow.log_params({"strategy_name": run.strategy_name, "factor_set_hash": run.factor_set_hash})
        ...
        return tracking_uri
    except Exception as exc:  # pragma: no cover - depends on optional local mlflow runtime.
        fallback = config.lake_root / config.lake.artifact_path / "mlflow.UNSPECIFIED_UNAVAILABLE.txt"
...
def strategy_decide(
    ...
    factor_values = [value for value in lake.read_factor_values() if value.symbol == symbol.upper()]
    decision = risk_check(select_decision(strategy_name, symbol, packets, factor_values), max_notional=max_notional)
    execution = execution_report_for_decision(decision, broker_permission=config.broker_permission)
    if not dry_run:
        output = config.lake_root / config.lake.artifact_path / "decisions" / f"{decision.id}.json"
        lake.write_json_artifact(output, decision)
        lake.write_json_artifact(output.with_name(f"{execution.id}.json"), execution)
    _echo({"dry_run": dry_run, "decision": decision.to_storage_dict(), "execution": execution.to_storage_dict()})
...
def report_render(
    ...
    report = render_constrained_report(
        ...
        _load_json_contracts(config.lake_root / config.lake.artifact_path / "backtests", BacktestRun),
        _load_json_contracts(config.lake_root / config.lake.artifact_path / "decisions", StrategyDecision),
    )
